...
func (c *CMSketch) queryHashValue(h1, h2 uint64) uint64 {
	...
	for i := range c.table {
		...
		x := c.table[i][j]
		noise := (c.count - uint64(x)) / uint64(c.width-1)
		if x == 0 {
			vals[i] = 0
		} else if uint64(x) < noise {
			vals[i] = temp
		} else {
			vals[i] = uint64(c.table[i][j]) - noise
		}
	}
	...
	res := vals[len(vals)/2]
	if len(vals)%2 == 0 {
		res = vals[len(vals)/2-1] + vals[len(vals)/2]/2
	}
	if uint64(min) < res {
		res = uint64(min)
	}
